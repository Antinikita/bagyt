...
function StatTile({ icon: Icon, label, value, unit, tint }) {
  return (
    <div className="flex items-center gap-3 rounded-xl border border-gray-100 bg-white p-3 dark:border-deep-700 dark:bg-deep-800">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
          <StatTile
            ...
            value={summary.steps !== null ? Math.round(summary.steps).toLocaleString() : null}
            tint="bg-brand-50 text-brand-700 dark:bg-deep-700 dark:text-brand-300"
          />
          <StatTile
            icon={Heart}
            label={t('health.avgHeartRate')}
            value={summary.avg_heart_rate !== null ? Math.round(summary.avg_heart_rate) : null}
            unit={t('health.bpm')}
          ...
          <StatTile
            ...
            value={formatSleep(summary.sleep_minutes)}
